 ...
 * For more detail about settings, @see https://github.com/luin/ioredis/blob/master/API.md#new-redisport-host-options
...
function IORedis(settings, dataSource) {
  ...
  // settings.sentinels = settings.sentinels || true;
...
IORedis.prototype.create = function create(model, data, options, callback) {
  ...
  if (id !== undefined) {
    promise = connection.call('sadd', ['s:' + model, id]);
  } else {
    // Get next id index of key
    promise = connection.call('incr', 'id:' + model).then(function (id) {
      tmpData[idName] = id.toString();
      return connection.call('sadd', ['s:' + model, tmpData[idName]]);
    });
  }

  return promise.then(function (res) {
    return this.save(model, tmpData, options, callback);
...
IORedis.prototype.save = function save(model, data, options, callback) {
  ...
  // TODO: Data format regulating for DB

  // deleteNull
  Object.keys(tmpData).forEach(function (key) {
    if (tmpData[key] === null) delete tmpData[key];
  });

  var promise = connection.call('hgetall', model + ':' + id).then(function (prevData) {
    return connection.call('hmset', [model + ':' + id, tmpData]).then(function () {
      return data;

      // TODO: Update index
      // if (prevData) {
      //   Object.keys(prevData).forEach(function (k) {
      //     if (tmpData.hasOwnProperty(k)) return;
      //     tmpData[k] = prevData[k];
      //   });
      // }
    });
  });
...
IORedis.prototype.updateOrCreate = function upsert(model, data, options, callback) {
  ...
  var promise = this.save(model, tmpData, options, callback).then(function (obj) {
    return connection.call('get', 'id:' + model).then(function (modelId) {
      if (!modelId) {
        return connection.call('set', ['id:' + model, id]);
      }
      return obj;
    });
  });
